...
const verify = async () => {
  ...
  if (!VERSION) {
    console.log(
      chalk.yellowBright(
        '请执行npm view @wemo-ui/marrs,查看当前tag版本, 确定版本;'
      )
    );
...
const runClean = async () => {
  ...
  await rimraf(distPath, function (err) {
    if (err) {
      console.log('clean dist 失败:', err);
    }
    console.log('clean dist 成功...');
  });
};

const runBuild = async () => {
  console.log('正在执行编译，请稍后...');
